  ...
  <div class="featured-content">
    <div class="row-fluid">
     <div class="addashmain">
	      ...
	      <?php
            if(!$msg)
              echo "<p style='color:mintcream;'><i>You Are Not Connected in Any Group. Please Connect Now</i></p>";
            else{
            	$i=0;
	      ...
	      <a href="javascript-camera-capture-master"><input type="submit" name="cg" value="Connect To Group" name=""></a>
    <?php
   ...
   <div class="featured-content">
    ...
    if($result){
    	while ($value = $result->fetch_assoc()) {
         ...
         if($dt1>$st){
         	$examcon = "Exam is Not Started";
          $ck = '';
         }elseif($dt1<$st && $newtime>$st){
         	$examcon = "Exam is Running";
          $ck = '';
         }elseif($st>$newtime){
         	$examcon = "Exam was Finished";
          $ck1 = '';
         }else{
         	$examcon = "";
         }
    ?>
	    <div class="row-fluid eachgr">
	      ...
	      <p style="margin-top: -12px;"><strong>Current Time: </strong><span style="color:mintcream"><?php echo $fm->DateFormat($st);?></span></p>
           ...
           <i><strong> Time Per Question: </strong><span style="color:mintcream"><?php echo $res['eachQuestionTime'];?> seconds</span></i>
	         </p>
           <p style="margin-top: -12px;">
             <i><strong> Total Time: </strong><span style="color:mintcream"><?php echo ($res['totalExamShowQues']*$res['eachQuestionTime']); ?> Seconds</span></i>
           </p>
            <?php
                 if($ck1=='f') {?>
           <a href="examhall.php?egid=<?php echo $group_id ;?>&gtok=<?php echo $group_token;?>"><input type="submit" name="viewgrp" value="Enter Into The Exam Hall"></a>
            <?php } ?>
	          <?php 
                 if($ck=='f') {?>
                 <?php
                 ...
                 if(!$cnfm)  { ?>
                   <input onclick = "confirm('Result is Not Published Yet')" type="submit" name="addquestion" value="Result">
                <?php } else{
                 ?>
