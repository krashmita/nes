   ...
   <div class="featured-content">
    ...
    $result = $gp->checkadminhasgroup($adminid,$regid);
        if($result){
         ...
         $newtime =  $datatime->format('Y-m-d H:i:sa');
         if($dt1>$st){
    ...
    ?>
	    <div class="row-fluid eachgr">
	      ...
	      <p style="margin-top: -12px;"><strong>Current Time: </strong><span style="color:mintcream"><?php echo $fm->DateFormat($st);?></span></p>
	         ...
	         <p style="margin-top: -12px;">
	         <i><strong>Question Limit: </strong><span style="color:mintcream"><?php echo $value['totalQuestion']; ?></span></i>
	         <i><strong> Show Question: </strong><span style="color:mintcream"><?php echo $value['totalExamShowQues']; ?></span></i>
